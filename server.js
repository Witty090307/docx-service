...
async function buscarImagenWikimedia(query){
  try {
    // 1. buscar archivos de imagen en commons
    const apiBusqueda = 'https://commons.wikimedia.org/w/api.php?action=query&format=json&generator=search'
      + '&gsrsearch=' + encodeURIComponent('filetype:bitmap|drawing ' + query)
      + '&gsrnamespace=6&gsrlimit=5&prop=imageinfo&iiprop=url|mime|size&iiurlwidth=900';
    const data = await getJSON(apiBusqueda);
    if(!data || !data.query || !data.query.pages) return null;

    const pages = Object.values(data.query.pages);
    // elegir la primera imagen apta (png/jpg, tamaño razonable)
    ...
    console.error('Error buscando imagen:', e && e.message);
    return null;
  }
}
...
async function resolverImagenes(data){
  ...
  for(const b of data.bloques){
    if(b && b.tipo === 'imagen' && b.query && !b.ruta){
      ...
      if(ruta){ b.ruta = ruta; rutasTemp.push(ruta); }
      // si no se encontró, el bloque se queda sin ruta y el generador lo ignora
    }
  }
  return rutasTemp;
}

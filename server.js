...
function buildPDF(res, data){
  ...
  // ===== PORTADA (banda de color) con padding cómodo =====
  const padTop = 52;
  // --- Pasada 1: medir la altura que ocupará el texto ---
  let measY = padTop;
  doc.fontSize(11); if(materia){ measY += doc.heightOfString(materia.toUpperCase(),{width:CW,characterSpacing:3}) + 8; }
  doc.font('Helvetica-Bold').fontSize(26);
  measY += doc.heightOfString(titulo||'Tarea',{width:CW,lineGap:1}) + 14;
  doc.fontSize(10.5);
  if(alumno) measY += doc.heightOfString('Alumno: '+alumno,{width:CW}) + 2;
  const l2=[]; if(grupo) l2.push('Grupo: '+grupo); if(plantel) l2.push('Plantel: '+plantel);
  if(l2.length) measY += doc.heightOfString(l2.join('     '),{width:CW}) + 2;
  if(fecha) measY += doc.heightOfString('Fecha: '+fecha,{width:CW});
  const bandH = measY + 34;            // padding inferior cómodo

  // --- Pintar la banda con la altura medida ---
  doc.rect(0,0,PW,bandH).fill(COLOR);

  // --- Pasada 2: escribir el texto encima ---
  let y = padTop;
  if(materia){
  ...
  if(alumno){ doc.font('Helvetica-Bold').text('Alumno: ',M,y,{continued:true}).font('Helvetica').text(alumno); y=doc.y+2; }
  if(l2.length){ doc.font('Helvetica').text(l2.join('     '),M,y); y=doc.y+2; }
  if(fecha){ doc.font('Helvetica-Bold').text('Fecha: ',M,y,{continued:true}).font('Helvetica').text(fecha); }

  doc.y = bandH + 30;                  // espacio después de la portada

...
async function intentarBusqueda(query){
  try {
    ...
    console.log('[IMG]   resultados:', pages.length);
    let candidatos = 0;
    for(const pg of pages){
      const info = pg.imageinfo && pg.imageinfo[0];
      if(!info) continue;
      const mime = info.mime || '';
      if(!/image\/(png|jpeg|jpg)/i.test(mime)) continue;  // solo png/jpg
      ...
      try {
        ...
        if(fs.existsSync(destino) && fs.statSync(destino).size > 2000){ console.log('[IMG]   descargada OK'); return destino; }
        try{ fs.unlinkSync(destino); }catch(e){}
      } catch(e){ console.log('[IMG]   falló descarga:', e && e.message); }
    }
    console.log('[IMG]   candidatos png/jpg:', candidatos, '(ninguno se pudo descargar)');
    return null;
  } catch(e){

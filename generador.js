...
async function generar(data, rutaSalida){
  ...
  for(const b of (data.bloques||[])){
    if(b.tipo==='h2') h2(b.texto);
    else if(b.tipo==='p') parrafo(b.texto);
    else if(b.tipo==='lista') lista(b.items);
    else if(b.tipo==='tabla') tabla(b.headers, b.rows);
    else if(b.tipo==='imagen'){
      // Solo se mete la imagen si viene una RUTA LOCAL que existe en disco.
      // Si trae 'query' o 'url' (aún no descargada), se ignora SIN colgar.
      if(b.ruta && typeof b.ruta==='string' && !/^https?:/i.test(b.ruta) && fs.existsSync(b.ruta)){
        imagen(b.ruta, b.caption, b.maxH);
      }
      // si no, se omite el bloque (el documento sale sin esa imagen)
    }
    else if(b.tipo==='diagrama_er') diagramaER(doc, b, {M,CW,P,INK,BODY,LINE,WHITE,BOTTOM,rr,shadow,nuevaPagina,PH});
  }

  doc.end();
  return new Promise(res=> doc.on('finish'? 'finish':'end', res) );
}
...
function diagramaER(doc, b, ctx){
  ...
  const ents = b.entidades||[];
  const n = ents.length;
  const gap=24;
  const boxW = Math.min(150, (CW-(n-1)*gap)/n);
  // estimar alto
  const maxCampos = Math.max(...ents.map(e=>e.campos.length));
  const boxH = 30 + maxCampos*16 + 10;
  ...
  ents.forEach((e,idx)=>{
    ...
    doc.save(); doc.roundedRect(x,startY,boxW,26,10).clip(); doc.rect(x,startY,boxW,26).fill(P.main); doc.restore();
    doc.font('Helvetica-Bold').fontSize(9.5).fillColor(WHITE).text(e.nombre, x, startY+8, {width:boxW, align:'center'});
    // campos
    let cy=startY+32;
    e.campos.forEach(c=>{
      doc.font('Helvetica').fontSize(8).fillColor(BODY).text('• '+c, x+10, cy, {width:boxW-16});
      cy+=16;
    });

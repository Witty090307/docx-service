  ...
  for(let bi=0; bi<bloques.length; bi++){
    ...
    try {
      ...
      if(b.tipo==='h2' && b.texto){
        // LOOKAHEAD: si el siguiente bloque es un párrafo, medimos cuánto ocupa
        // para que el título no quede huérfano al final de la hoja.
        const sig = bloques[bi+1];
        let altoSig = 0;
        if(sig && sig.tipo==='p' && sig.texto){
          altoSig = medirParrafo(String(sig.texto)) + 10;
        } else if(sig && sig.tipo==='lista' && Array.isArray(sig.items)){
          altoSig = Math.min(60, sig.items.length*16); // al menos parte de la lista
        }

...
async function generar(data, rutaSalida){
  ...
  let sec=0;
  // dibuja el título SIN decidir salto (el salto lo maneja quien llama)
  function dibujarH2(t){
    sec++;
    const yy=doc.y+4;
  ...
  // mide cuánto ocupa un título (chip + texto)
  function medirH2(t){
    const altoTexto = doc.font('Helvetica-Bold').fontSize(15).heightOfString(t, {width:CW-32});
    return Math.max(22, altoTexto) + 12; // alto del chip o del texto + espacio
  }
  function h2(t, altoSiguiente){
    // si viene un bloque que debe quedar pegado (ej. su párrafo),
    // medimos título + ese bloque juntos para no dejar el título huérfano
    const altoTitulo = medirH2(t);
    const altoJunto = altoTitulo + (altoSiguiente || 0);
    // si juntos no caben pero sí caben en una página nueva, saltamos ANTES del título
    if(doc.y + altoJunto > BOTTOM && altoJunto < (PH-100)){ nuevaPagina(); }
    else { espacio(altoTitulo); }
    dibujarH2(t);
  }
  ...
  function parrafo(txt){
    ...
    // REGLA mejorada: el párrafo salta entero solo si de verdad no cabe.
    // Damos un margen: si caben al menos ~3 renglones del párrafo donde estamos,
    // no saltamos (evita huecos grandes por mover párrafos que casi caben).
    const espacioRestante = BOTTOM - doc.y;
    const unRenglon = 14;
    if(h > espacioRestante){
      // no cabe completo. ¿Cabe en una página nueva entero?
      if(h < (PH-120)){
        // si lo que queda es muy poco (menos de ~3 renglones de hueco), saltamos;
        // pero si el hueco fuese grande, igual saltamos porque el párrafo no se parte.
        nuevaPagina();
      }
      // si es gigantesco (más que una página), se deja fluir y PDFKit lo parte solo.
    } else {
      espacio(20);
    }
  ...
  function imagen(ruta, caption, maxH){
    ...
    try {
      ...
      // tamaño ideal (ancho completo, limitado por maxH)
      let w = CW, h = w/ratio;
      if(h>maxH){ h=maxH; w=h*ratio; }

      const espacioRestante = BOTTOM - doc.y;        // lo que queda en la hoja actual
      const ALTO_MIN_LEGIBLE = 120;                  // si tuviera que achicarse a menos de esto, mejor salta
      let totalH = h + 16;

      if(totalH > espacioRestante){
        // no cabe a tamaño ideal. ¿Se puede ESCALAR para caber y sigue legible?
        const hPosible = espacioRestante - 16;       // alto de imagen que cabría en el hueco
        if(hPosible >= ALTO_MIN_LEGIBLE){
          // sí cabe escalada y sigue siendo legible -> la achicamos para llenar el hueco
          h = hPosible;
          w = h*ratio;
          if(w > CW){ w = CW; h = w/ratio; }         // por si el ancho se pasa
          totalH = h + 16;
        } else {
          // escalarla la dejaría muy chica -> mejor saltar a la siguiente hoja a tamaño bueno
          nuevaPagina();
          // recalcular a tamaño ideal en la hoja nueva
          w = CW; h = w/ratio;
          if(h>maxH){ h=maxH; w=h*ratio; }
          totalH = h + 16;
        }
  ...
  for(let bi=0; bi<bloques.length; bi++){
    ...
    try {
      ...
      if(b.tipo==='h2' && b.texto){
        // LOOKAHEAD: medimos el INICIO del siguiente bloque (sea del tipo que sea)
        // para que el título no quede huérfano al final de la hoja.
        const sig = bloques[bi+1];
        let altoSig = 0;
        if(sig){
          if(sig.tipo==='p' && sig.texto){
            altoSig = Math.min(medirParrafo(String(sig.texto)) + 10, 90); // basta que entre el inicio
          } else if(sig.tipo==='lista' && Array.isArray(sig.items)){
            altoSig = Math.min(60, sig.items.length*16);
          } else if(sig.tipo==='diagrama_flujo' && Array.isArray(sig.pasos) && sig.pasos.length){
            // altura real COMPLETA del diagrama (no se parte si cabe en una página)
            const bw = Math.min(360, CW-80), pX=16, pY=11, gapF=26;
            doc.font('Helvetica').fontSize(10.5);
            let altoFlujo = 0;
            sig.pasos.forEach((p,i)=>{
              altoFlujo += doc.heightOfString(String(p), {width: bw-pX*2-30}) + pY*2;
              if(i<sig.pasos.length-1) altoFlujo += gapF;
            });
            // tope a una página entera (si es más grande, igual se partirá)
            altoSig = Math.min(altoFlujo, PH-160);
          } else if(sig.tipo==='diagrama_er' && Array.isArray(sig.entidades) && sig.entidades.length){
            const maxC = Math.max(1, ...sig.entidades.map(e=>(e&&Array.isArray(e.campos))?e.campos.length:0));
            altoSig = Math.min(30 + maxC*15 + 10 + 20, PH-160);
          } else if(sig.tipo==='tabla' && Array.isArray(sig.headers)){
            altoSig = 60;  // header + primera fila aprox
          } else if(sig.tipo==='imagen' && sig.ruta){
            altoSig = 130; // alto mínimo legible de imagen
          }
        }
        h2(String(b.texto), altoSig);
      }
...
function diagramaFlujo(doc, b, ctx){
  ...
  // 1) MEDIR todo el diagrama primero (altura total de pasos + flechas)
  doc.font('Helvetica').fontSize(fs);
  const alturas = pasos.map(p => doc.heightOfString(p, {width: boxW - padX*2 - 30}) + padY*2);
  const alturaTotal = alturas.reduce((a,b)=>a+b,0) + gapFlecha*(pasos.length-1);

  // 2) decidir salto: si no cabe completo pero cabe en una hoja nueva, saltar TODO junto
  //    (así no se corta a media línea/diagrama). Solo si es más alto que una página, se parte.
  const altoPagina = BOTTOM - 52;
  if(doc.y + alturaTotal > BOTTOM && alturaTotal <= altoPagina){
    nuevaPagina();
  }

  pasos.forEach((paso, i) => {
    const boxH = alturas[i];
    // si el diagrama es más grande que una hoja entera, sí permitimos partir
    // (pero entre cajas completas, nunca a media caja)
    if(doc.y + boxH + (i<pasos.length-1?gapFlecha:0) > BOTTOM){ nuevaPagina(); }
